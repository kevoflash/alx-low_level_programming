 ...
 *    eturn: void
 *     */
void check99(ssize_t check, char *file, int fd_from, int fd_to)O, "Error: Can't write to %s\n", file);
		if (fd_from != -1)
				close(fd_from);
						if (fd_to != -1)
				close(fd_to);
 ...
 *     * Return: 0 on success
 *      */
 int main(int argc, char *argv[]enr, lenw;
		 	char buffer[1024];
				mode_t file_perm;
							...
							check98((ssize_t)fd_from, argv[1], -1, -1);
								file_perm = S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH;
									fd_to = open(argv[2], O_WRONLY | O_CREAT | O_TRUNC, file_perm);
										check99((ssize_t)fd_to, argv[2], fd_from, -1);om, buffer, 1024);
